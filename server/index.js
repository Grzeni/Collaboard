...
io.on("connect", socket => {
    ...
    socket.on('new-user-connected', ({ username, room }, callback) => {
        ...
        io.in(user.room).emit('connect-text-emition', Array.from(text_event_map));
        io.in(user.room).emit('connect-drawing-emition', drawing_event_array);
        console.log('connected to new user, sending out drawing data', drawing_event_array);
